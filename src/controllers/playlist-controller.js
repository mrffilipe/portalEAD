...
exports.store = async (req, res) => {
    try {
        ...
        if (!id || id == null) return res.status(404).send({ message: 'User not found!' });
...
exports.index = async (req, res) => {
    try {
        ...
        if (allPlaylist == 0) return res.status(404).send({ message: 'Playlist not found!' });
...
exports.update = async (req, res) => {
    try {
        ...
        if (!playlistName) return res.status(406).send({ message: 'Playlist name cannot be empty!' });

        if (await knex('playlist').where({ id: idplaylist, _idUser: id }).update('name', playlistName) == 0) return res.status(404).send({ message: 'No records were found with the data entered!' });
...
exports.delete = async (req, res) => {
    try {
        ...
        if (await knex('playlist').where({ id: idplaylist, _idUser: id }).del() == 0) return res.status(404).send({ message: 'No records were found with the data entered!' });
